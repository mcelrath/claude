...
|------|---------|--------------|
| **block-text-search-on-source** | `grep`/`rg`/`find`/`awk`/`sed` on source files (.py, .lean, .md, etc.) | Python → `ast-grep --lang python --pattern '$X'`; markdown → `ast-grep run -c ~/.config/ast-grep/sgconfig.yml --lang markdown --pattern '$X'`; other → the `Read` tool; symbol usage → LSP (see *Read Before You Write & Searching*). Lean search tooling (loogle / lean-search / lean-audit) → see the project `CLAUDE.md`. |
