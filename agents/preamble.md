...
4. **Verify against code, don't infer from claims**. To know whether something was done, READ the code/test/data that settles it — never infer from a TODO comment, a doc, or an **issue-tracker field**. Comments, docs, and **bd issue titles / notes / status all describe what was true WHEN WRITTEN**; they are CLAIMS to verify, never ground truth. This is doubly true for **audit / reconciliation / "is this still open?" work: the tracker is the QUESTION, not the answer** — a bug titled "X is broken" may have been fixed long ago. Read the relevant code IN FULL (not a grep line, not a skim) — code and data don't go stale. (Failure this encodes: a reconciliation agent kept an IMU issue "open" by trusting its stale note "missing program_rlc_ram" — the function was implemented in `imu.rs` all along; reading the code, not the note, was the answer.)
